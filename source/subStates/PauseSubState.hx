	...
	public function new()
	{
		...
		if(PlayState.SONG.composer != null && PlayState.SONG.arranger != null) {
			if(PlayState.SONG.composer == PlayState.SONG.arranger) {
				textArray.push("Composed and Remixed by: " + PlayState.SONG.composer);
				alreadyDone = true;
			}
		}

		if(!alreadyDone) {
			if(PlayState.SONG.composer != null)
				textArray.push("Composed by: " + PlayState.SONG.composer);
			if(PlayState.SONG.arranger != null)
				textArray.push("Remixed by: " + PlayState.SONG.arranger);
		}

		textArray.push('BLUEBALLED: ' + PlayState.blueballed);
